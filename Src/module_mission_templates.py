...
battle_siege_equalize_division = (
    ...
    [],
	[
		(call_script, "script_team_equalize_divisions", 0),
        (try_begin),
            (eq, "$g_player_team", 0),
			(call_script, "script_team_equalize_divisions", 2),
        (try_end),
	])
	...
	0, 0, ti_once,
		...
		[
			...
			(try_for_range, ":division", grc_infantry, 9),
				...
				(team_give_order, 0, ":division", mordr_stand_closer),
			(try_end),
			(call_script, "script_set_team_defend_points", 0),
		])
	...
	0, 0, ti_once,
		...
		[
            ...
            (try_begin),
				...
				(team_give_order, 2, grc_archers, mordr_stand_ground),
				
				(try_for_range, ":division", grc_infantry, 9),
					(team_give_order, 2, ":division", mordr_stand_closer),
					(team_give_order, 2, ":division", mordr_stand_closer),
				(try_end),

				(call_script, "script_set_team_defend_points", 2),
            (try_end),
		])
	...
	60, 0, 0,
		...
		[
			(try_for_agents, ":agent_no"),
				...
				# only change positions of defending agents
				(agent_get_division, ":division", ":agent_no"),
...
mission_templates = [
		...
		[
			...
			(ti_before_mission_start, 0, ti_once,
				...
				[
					...
					(team_set_slot, 0, slot_team_battle_phase, stbp_siege_one),
					(team_set_slot, 1, slot_team_battle_phase, stbp_siege_one),
					(team_set_slot, 2, slot_team_battle_phase, stbp_siege_one),
				]),
			...
			(ti_after_mission_start, 0, ti_once,
				...
				[
					...
					(try_begin),
						(eq, "$g_player_team", 0),
						(team_set_slot, 2, slot_team_formation, stf_siege),
						(call_script, "script_team_set_division_slots_for_formation", 2, stf_siege),
					(else_try),
						(team_set_slot, 2, slot_team_formation, stf_default),
						(call_script, "script_team_set_division_slots_for_formation", 2, stf_default),
					(try_end),
	...
	("arena_tournament", mtf_arena_fight, 0, "Tournament",
		[
			(1, mtef_visitor_source|mtef_team_0, af_override_all, aif_start_alarmed, 1, []),
			(2, mtef_visitor_source|mtef_team_1, af_override_all, aif_start_alarmed, 1, []),
			(3, mtef_visitor_source|mtef_team_2, af_override_all, aif_start_alarmed, 1, []),
			(4, mtef_visitor_source|mtef_team_3, af_override_all, aif_start_alarmed, 1, []),
			(5, mtef_visitor_source|mtef_team_0, af_override_all, aif_start_alarmed, 1, []),
			(6, mtef_visitor_source|mtef_team_1, af_override_all, aif_start_alarmed, 1, []),
			(7, mtef_visitor_source|mtef_team_2, af_override_all, aif_start_alarmed, 1, []),
		...
		[
			...
			(ti_after_mission_start, 0, 0, [],
				[
					...
					# we use a temporary number generated from the menu because the first number generated in a scene is always the same
                    (assign, ":rand", "$g_temp"),

                    (assign, reg10, ":rand"),
                    (display_message, "@Rand: {reg10}"),

                    (try_for_range, ":team", 0, 4),
                        (team_set_slot, ":team", slot_team_entry_seed, ":rand"),
                        (team_set_slot, ":team", slot_team_entry_offset, 0),
                    (try_end),
					...
					(try_begin),
						(scene_get_slot, ":num_visitors", ":scene", slot_scene_visitors_count),
						(gt, ":num_visitors", 0),
						(modify_visitors_at_site, ":scene"),
						(try_for_range, ":cur_visitor", 0, ":num_visitors"),
							(store_mul, ":visitor_slot", ":cur_visitor", 2),
							(val_add, ":visitor_slot", slot_scene_visitors_begin),
							(store_add, ":entry_slot", ":visitor_slot", 1),

							(scene_get_slot, ":troop_no", ":scene", ":visitor_slot"),
							(scene_get_slot, ":entry", ":scene", ":entry_slot"),

							(team_get_slot, ":seed", ":entry", slot_team_entry_seed),
							(team_get_slot, ":offset", ":entry", slot_team_entry_offset),

							(store_add, ":offset_entry", ":entry", ":offset"),

		                    (call_script, "script_party_get_tournament_round_type", "$g_encountered_party", ":active_participants"),
		                    (assign, ":round_type", reg0),
							(call_script, "script_party_set_tournament_round_override", "$g_encountered_party", ":round_type", ":offset_entry", ":seed"),
							(add_visitors_to_current_scene, ":offset_entry", ":troop_no", 1),

							(val_add, ":seed", 1),
							(team_set_slot, ":entry", slot_team_entry_seed, ":seed"),
							(val_add, ":offset", 8),
							(team_set_slot, ":entry", slot_team_entry_offset, ":offset"),
						(try_end),
					(try_end),
				]),
