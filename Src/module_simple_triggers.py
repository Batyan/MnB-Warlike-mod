...
simple_triggers = [
        ...
        [
            (try_for_parties, ":party_no"),
                ...
                (else_try),
                    ...
                    (call_script, "script_party_process_camp", ":party_no"),
                (try_end),
        ...
        [
            (try_for_parties, ":party_no"),
                ...
                (else_try),
                    ...
                    (try_end),
                (else_try),
                    (eq, ":party_type", spt_garrison),
                    (call_script, "script_party_garrison_process", ":party_no"),
                (try_end),
            (try_end),
        ]),
    ...
    (0.1, # Weather
        [
            (try_for_parties, ":party_no"),
                ...
                (party_get_cur_town, ":cur_town", ":party_no"),
                (try_begin),
                    (this_or_next|is_between, ":cur_town", centers_begin, centers_end),
                    (is_between, ":cur_town", camps_begin, camps_end),
                    (try_begin),
        ...
        [
            ...
            (try_begin),
                ...
                (try_begin),
                    ...
                    (store_add, ":next_message", "$g_player_prisoner_last_message", 30),
                    (gt, ":current_day", ":next_message"),
                    (display_message, "@You are being held captive.", text_color_info),
                    (display_message, "@Hold space to open escape menu.", text_color_info),
    ...
    (monthly,
        [
            (assign, ":has_joined_player", 0),
            (try_for_range, ":npc", npc_heroes_begin, npc_heroes_end),
                (troop_get_slot, ":occupation", ":npc", slot_troop_kingdom_occupation),
                (eq, ":occupation", tko_follower),

                (troop_get_slot, ":companion_of", ":npc", slot_troop_companion_of),
                (this_or_next|eq, ":companion_of", "$g_player_troop"),
                (is_between, ":companion_of", npc_heroes_begin, npc_heroes_end),

                (str_store_troop_name, s10, ":npc"),
                (str_store_troop_name, s11, ":companion_of"),
                (display_message, "@{s10} is companion of {s11}"),

                (troop_get_slot, ":prisoner_of", ":npc", slot_troop_prisoner_of),
                (le, ":prisoner_of", -1),
                (display_message, "@{s10} not a prisoner"),

                (assign, ":continue", 0),
                (try_begin),
                    (eq, ":companion_of", "$g_player_troop"),
                    (display_message, "@{s10} leader is player"),
                    (troop_get_slot, ":prisoner_of", ":npc", slot_troop_prisoner_of),
                    (troop_get_slot, ":player_prisoner", "$g_player_troop", slot_troop_prisoner_of),
                    (eq, ":player_prisoner", -1),
                    (display_message, "@{s10} leader is not prisoner"),
                    (assign, ":continue", 1),
                (else_try),
                    (troop_get_slot, ":leader_party", ":companion_of", slot_troop_leaded_party),
                    (ge, ":leader_party", 0),
                    (display_message, "@{s10} leader has a party"),
                    (party_is_active, ":leader_party"),

                    (display_message, "@{s10} leader's party is active"),
                    (assign, ":continue", 1),
                (try_end),
                (eq, ":continue", 1),
                ...
                (try_for_range, ":cur_stack", 0, ":num_stacks"),
                    (party_stack_get_troop_id, ":troop_id", ":leader_party", ":cur_stack"),
                    (eq, ":troop_id", ":npc"),
                ...
                (eq, ":has_companion", 0),
                (display_message, "@{s10} is not in leader's party"),
                (try_begin),
                    (eq, ":companion_of", "$g_player_troop"),
                    (display_message, "@{s10} is starting meeting"),
                    (eq, ":has_joined_player", 0),
                    (assign, "$temp", ":npc"),
                    (jump_to_menu, "mnu_encounter_talk"),
                    (assign, ":has_joined_player", 1),
                (else_try),
                    (party_add_members, ":leader_party", ":npc", 1),
                (try_end),
            (try_end),
        ]),
]
